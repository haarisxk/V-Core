...
# Force Windows console to use UTF-8 so special characters don't break things
...
def elevate():
    if sys.platform != "win32":
        ...
        sys.exit(1)
    # need to wrap the args in quotes so spaces in the path don't break the runner
...
class ValorantTerminal:
    def __init__(self, target_agents: List[str], auto_lock: bool = True, is_stealth: bool = False):
        ...
        # stuff we only need to fetch once from valorant-api
        ...
        # transient caches (we clear these between games)
    ...
    def _resolve_mode(self, match: Dict[str, Any]) -> str:
        ...
        # 1. try raw UUID (usually works in CoreGame)
        if mode_id:
        ...
        # 2. try parsing the asset path string
        ...
        # 3. fallback to checking the full path string
        name = self.mode_lookup.get(raw_mode)
        if name:
            return name

        # 4. look at provisioning flow hints
        ...
        # 5. last resort: look at matchmaking queue data
    ...
    def run(self) -> None:
        ...
        # hook F8 if the keyboard module actually installed correctly
        ...
        # translate our agent string into raw UUIDs
        self.target_uuids = []
        ...
        while True:
                    ...
                    if match:
                        ...
                        if self.auto_lock and self.target_uuids and not self.lock_success:
                            for agent_uuid, agent_name in self.target_uuids:
                                ...
                                # 2) see if someone else on our team already locked it
                                our_team = None
                                ...
                                if our_team:
                                    ...
                                    if agent_uuid.lower() in locked_agents:
                                        continue  # damn, it's taken. cascade down the list.

                                # 3) small randomized delay so it doesn't look like a bot
                                ...
                                if success:
                                    ...
                                    # got it! force a ui redraw
                                    match = self.get_pregame_match()
                                    if match:
                                        self.cached_match_display = copy.deepcopy(match)
                                        self.draw_match_info(match, is_ingame=False)
                                    break  # secured our agent, exit the waterfall
                                else:
                                    # lock failed (maybe someone beat us to it in the last few ms)
                                    continue
                        # ------------------------------------------------
            ...
            # handle timing and state-specific redraws
            if state == "MENUS":
    ...
    def print_store(self) -> None:
        ...
        try:
            ...
            # grab mapping tables so we know what the UUIDs actually mean
    ...
    try:
        ...
        if keyboard is not None:
            try:
                ...
                pass
        # cleanup our firewall rules so the user isn't permanently stuck offline
        if args.stealth:
            disable_stealth()
